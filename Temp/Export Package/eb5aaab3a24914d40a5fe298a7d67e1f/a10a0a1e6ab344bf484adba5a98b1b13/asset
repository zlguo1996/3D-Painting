Shader "VTP/Metallic/2 Texture Blend (Advanced)" {
	Properties {
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			//if( IN.uv_MainTex.x < 0 || IN.uv_MainTex.x > 1 || IN.uv_MainTex.y < 0 || IN.uv_MainTex.y > 1 )
			//	clip(-1);
			...
			if( (IN.flowUV.x == 0 && IN.flowUV.y == 0) || (_useFlowMap2 == 0 ) || _useFlowMapGlobal == 0 ) {
				...
				//Normal w nCF
				_normal = normalCorrectionFactor * (UnpackNormal (tex2Dlod (_BumpMap, float4(IN.uv_MainTex,0,mipL1))) * normalFactorRed + UnpackNormal (tex2Dlod (_BumpMap2, float4(IN.uv_MainTex2,0,mipL2)))* normalFactorGreen  ); 	                 

				//Albedo w CF
				_albedo = correctionFactor * ( tex2Dlod (_MainTex, float4(IN.uv_MainTex,0,mipL1)).rgb * factorRed + tex2Dlod (_MainTex2, float4(IN.uv_MainTex2,0,mipL2) ).rgb * factorGreen  );
		        ...
		        //Smoothness w CF     
		        _smoothness = normalCorrectionFactor * ( _Smoothness * combinedMapData.b * normalFactorRed + _Smoothness2 * combinedMap2Data.b * normalFactorGreen  ) ;
					
				//Occlusion w CF
				_occlusion =  correctionFactor * ( lerp(1, combinedMapData.g, _Occlusion) * factorRed  + lerp(1, combinedMap2Data.g, _Occlusion2) * factorGreen  );
					
				//Metallic
				_metallic =  correctionFactor * ( _Metallic * tex2Dlod (_MainTex, float4(IN.uv_MainTex,0,mipL1)).a * factorRed + _Metallic2 * tex2Dlod (_MainTex2, float4(IN.uv_MainTex2,0,mipL2)).a * factorGreen );

				_emission =  correctionFactor * ( _EmissionColor * _Emission * combinedMapData.a * factorRed + _EmissionColor2 * _Emission2 * combinedMap2Data.a * factorGreen );
				...
				fixed3 normal1 = ( normalDataRed + normalDataGreen1 );  
				fixed3 normal2 = ( normalDataRed + normalDataGreen2 );  
				...
				_emission =  correctionFactor * ( _EmissionColor * _Emission * combinedMapData.a * factorRed + _EmissionColor2 * _Emission2 * combinedMap2Data.a * factorGreen );
					...
					fixed3 albedo1 = ( albedoDataRed + albedoDataGreen1 );  
					fixed3 albedo2 = ( albedoDataRed + albedoDataGreen2 );  
			        ...
			        //Smoothness w CF     
			        fixed3 smoothness1 = ( _Smoothness * combinedMapDataRed.b * normalFactorRed +  _Smoothness2 * combinedMapDataGreen1.b * normalFactorGreen  );  
			        fixed3 smoothness2 = ( _Smoothness * combinedMapDataRed.b * normalFactorRed +  _Smoothness2 * combinedMapDataGreen2.b * normalFactorGreen  );  
			        _smoothness = correctionFactor * lerp( smoothness1, smoothness2, flowLerp );
						
					///Occlusion w CF
					fixed3 occlusion1 = ( lerp(1, combinedMapDataRed.g, _Occlusion) * factorRed +  lerp(1, combinedMapDataGreen1.g, _Occlusion2) * factorGreen  );  
			        fixed3 occlusion2 = ( lerp(1, combinedMapDataRed.g, _Occlusion) * factorRed + lerp(1, combinedMapDataGreen2.g, _Occlusion2) * factorGreen  );  
			        _occlusion = correctionFactor * lerp( occlusion1, occlusion2, flowLerp );

			        												
					//Metallic
					fixed3 metallic1 = ( _Metallic * tex2Dlod (_MainTex, float4(IN.uv_MainTex,0,mipL1)).a * factorRed +  _Metallic2 * tex2Dlod (_MainTex2, float4(flowUV2_phase0,0,mipL2)).a * factorGreen  );  
			        fixed3 metallic2 = ( _Metallic * tex2Dlod (_MainTex, float4(IN.uv_MainTex,0,mipL1)).a * factorRed +  _Metallic2 * tex2Dlod (_MainTex2, float4(flowUV2_phase1,0,mipL2)).a * factorGreen  );  
			        _metallic = correctionFactor * lerp( metallic1, metallic2, flowLerp );

			        //Emission
			        fixed3 emission1 = ( _EmissionColor * _Emission * combinedMapDataRed.a * factorRed + _EmissionColor2 * _Emission2 * combinedMapDataGreen1.a * factorGreen );  
			        fixed3 emission2 = ( _EmissionColor * _Emission * combinedMapDataRed.a * factorRed + _EmissionColor2 * _Emission2 * combinedMapDataGreen2.a * factorGreen );  
