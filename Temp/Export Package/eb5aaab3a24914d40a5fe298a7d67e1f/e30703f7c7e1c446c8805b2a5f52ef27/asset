...
Shader "VTP/V2/Tesselation/Specular" {
	Properties {
	...
	SubShader {
		...
		#pragma surface surf StandardSpecular fullforwardshadows addshadow vertex:disp tessellate:tess
		...
		tex_Blue_Phase = float4(tex_Blue,tex_Blue);
			...
			half4 specularGloss = lerp (
									SpecularGloss(tex_Red, tex_Green_Phase.xy, tex_Blue_Phase.xy, colorFactor),
									SpecularGloss(tex_Red, tex_Green_Phase.zw, tex_Blue_Phase.zw, colorFactor), 
									flowLerp);
			o.Specular = specularGloss.rgb;
		    o.Smoothness = specularGloss.a;
	...
	#if _WETNESS && TARGET30 == 1
			half4 specularGloss = SpecularGlossWithWetness(tex_Red, tex_Green, tex_Blue, colorFactor, wetnessSmoothnessModifiers);		
	#else
			half4 specularGloss = SpecularGloss(tex_Red, tex_Green, tex_Blue, colorFactor);	
	#endif	
	...
	FallBack "Diffuse"
	CustomEditor "ShaderEditor_Spec_Tesselation_V2"
}
